...
const projects = [
  ...
  {
    num: "03", tag: "CHURN INTELLIGENCE", year: "2025", title: "Customer Segmentation & Churn",
    ...
    metrics: [{ val: "2.5M+", label: "Transactions" }, { val: "0.8793", label: "ROC-AUC" }, { val: "$233K", label: "At-Risk" }, { val: "4", label: "Segments" }],
    github: "https://github.com/RishabhPR77/customer-churn", live: "https://customer-churn-dunhumby.streamlit.app/", color: "#00d4ff", thumb: "/thumbnails/churn.png",
  },
  {
  ...
  {
    num: "05", tag: "COMPUTER VISION", year: "2025", title: "Video Target ID",
    ...
    metrics: [{ val: "512-d", label: "Face Embed" }, { val: "12-d", label: "Pose Vec" }, { val: "0.7/0.3", label: "Fusion" }, { val: "Multi", label: "CCTV Scan" }],
    github: "https://github.com/RishabhPR77/video-target-id", live: "https://video-target-id-lnjmjrh3ap8jafch7wrbme.streamlit.app/", color: "#a855f7", thumb: "/thumbnails/vtis.png",
  },
];
      ...
      <main className="relative">
        ...
        <section className="relative z-10 min-h-screen flex flex-col justify-center px-6 md:px-12">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }} className="max-w-6xl mx-auto w-full">
            <div className="inline-flex items-center gap-3 rounded-full border border-[#00d4ff]/30 bg-[#00d4ff]/10 px-4 py-1.5 mb-3 text-[13px] tracking-[0.2em] text-[#00d4ff]">
    ...
    <section id="skills" className="relative z-10 px-6 md:px-12 py-32">
      <div className="max-w-6xl mx-auto grid md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-8">
          <Reveal>
            <h2 className="text-4xl md:text-6xl font-semibold tracking-tight leading-tight mb-3">Technical skills<span className="text-[#00d4ff]">.</span></h2>
            <p className="text-white/60 max-w-xl mb-10">Languages, frameworks and tools I work with — grouped by what they actually do.</p>
          </Reveal>
            ...
            {groups.map((grp, gi) => (
              <Reveal key={grp.g} from={gi % 2 === 0 ? "left" : "right"} delay={gi * 0.06}>
                <div className="group relative rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-6 overflow-hidden transition-colors hover:border-white/25 h-full">
                  <span className="pointer-events-none absolute -top-12 -right-12 w-40 h-40 rounded-full opacity-30 group-hover:opacity-60 transition-opacity blur-2xl" style={{ background: grp.c }} />
                  <div className="relative flex items-center gap-3 mb-5">
                    <span className="w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold font-mono" style={{ background: `${grp.c}1f`, color: grp.c, border: `1px solid ${grp.c}55` }}>{grp.icon}</span>
                    <div>
                  ...
                  <div className="relative flex flex-wrap gap-1.5">
                    {grp.items.map((it) => (
                      <span key={it} className="font-mono text-[11px] tracking-wide px-2.5 py-1 rounded-md border border-white/10 bg-black/30 text-white/75 hover:text-white transition-colors">{it}</span>
                    ))}
                  </div>
                </div>
              </Reveal>
...
function ContactCard({ k, v, h, c, icon, index }: { k: string; v: string; h: string; c: string; icon: string; index: number }) {
  ...
  return (
    <motion.a
      ...
      viewport={{ once: false, root: scrollRootRef, margin: "-10% 0px -10% 0px" }}
    ...
    >
      <span className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: `radial-gradient(420px circle at 0% 100%, ${c}22, transparent 60%)` }} />
      <div className="relative shrink-0 w-14 h-14 rounded-xl flex items-center justify-center border transition-all group-hover:scale-105" style={{ background: `${c}14`, borderColor: `${c}40`, color: c }}>
